if __name__ == '__main__':
    ...
    for i in range(total_p_no):
        wait.append(turnaround_time[i] - proc[i][2])


    print("{:<10} {:<10} {:<10} {:<10} {:<10}".format("Process", 'Arrival', 'Burst', 'Turnaround', 'Waiting'))
    for i in range(total_p_no):
        print("{:<10} {:<10} {:<10} {:<10} {:<10}".format(
            str(i), str(proc[i][0]), str(proc[i][1]), str(turnaround_time[i]), str(wait[i])))
